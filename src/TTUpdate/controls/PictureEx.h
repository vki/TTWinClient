...
public:
	...
	CSize GetControlSize(){
		...
		return CSize((nPaintWidth > 0)?min(nPaintWidth,m_PictureSize.cx):m_PictureSize.cx,
			(nPaintHeight > 0)?min(nPaintHeight,m_PictureSize.cy):m_PictureSize.cy);
	}
