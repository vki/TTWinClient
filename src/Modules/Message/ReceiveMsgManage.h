...
typedef std::list<MessageEntity> SessionChatMsg_List;
typedef std::map<std::string, SessionChatMsg_List> SessionChatMsgMap;
...
public:
	...
	BOOL popAllMessageBySId(const std::string& sId, SessionChatMsg_List& msgList);
...
private:
	SessionChatMsg_List* getChatMsgListBySID(const std::string& sId);
	virtual int outputUrlCallback(unsigned pos, const UrlScanner::STRING& url);
	void UrlReplace(CString& content);
	void UrlScan(CString& content);

private:
	SessionChatMsgMap           m_mapSessionChatMsg;
...
private:
	...
	SessionChatMsgMap           m_mapUnReadAudioMsg;
