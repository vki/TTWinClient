...
void UserListModule_Impl::getAllUsersInfo(module::UserInfoEntityMap& MapUsers)
{
	MapUsers = m_mapUsers;
}
...
void UserListModule_Impl::tcpGetUserFriendInfoList(IN const module::UserInfoEntityVec& VecUnKnowUserInfo)
{
...
void UserListModule_Impl::_tcpGetUserInfoList(IN module::UserInfoEntityVec VecUnKnowUserInfo)
{
	...
	int nCount = 0;
	module::UserInfoEntityVec::iterator it = VecUnKnowUserInfo.begin();
	...
	for (auto& kv : m_mapUsers)
	{
		...
		if (!user.avatarUrl.empty() && user.avatarUrl.find("avatar_default.jpg") == std::string::npos)
		{
			...
			if (!imageEntity.filename.empty() && util::isFileExist(csLocalPath))
			{
				...
				user.avatarGrayLocalPath = getGrayLocalPathFromFilename(imageEntity.filename);
			}
			else
			{
				//不存在则去服务器下载
...
void UserListModule_Impl::onCallbackOperation(std::shared_ptr<void> param)
{
	...
	if (DownloadImgParam::DOWNLOADIMG_OK == pDownParam->m_result)
	{
		...
		if (userIter != m_mapUsers.end())
		{
			...
			userIter->second.avatarGrayLocalPath = getGrayLocalPathFromFilename(pDownParam->m_imgEntity.filename);
		}
