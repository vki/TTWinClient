...
void SysConfigDialog::OnClick(TNotifyUI& msg)
{
	...
	else if (m_TestSoundBtn == msg.pSender)
	{
		...
		{
			module::getMiscModule()->playSound(module::getMiscModule()->getDataDir() + _T("Sound\\duoduo_dingdong.wav"));
		}
		else if (m_JianpanRdo->IsSelected())
		{
			module::getMiscModule()->playSound(module::getMiscModule()->getDataDir() + _T("Sound\\duoduo_keyboard.wav"));
		}
		...
		{
			module::getMiscModule()->playSound(module::getMiscModule()->getDataDir() + _T("Sound\\duoduo_callmsg.wav"));
		}
	}
...
void SysConfigDialog::_ApplySetting()
{
	...
	if (!pTTConfig)
	{
		APP_LOG(LOG_ERROR, _T("_ApplySetting,读取设置失败"));
		return;
	}
