...
CControlUI* FloatDialog::CreateControl(LPCTSTR pstrClass)
{
	return nullptr;
}
...
{
	DuiLib::CPoint m_ptSysPromptForm = m_LastPos;//获取上一次的位置
	int count = 0;
	...
	if (m_Created)
	{
		if (m_ptSysPromptForm.y - h - 10 < rWorkArea.top)//如果已经超过了上边界,则放到最下面
		{
...
{
	logic::GetLogic()->asynNotifyObserver(module::KEY_SESSION_OPENNEWSESSION, m_floatInfo.sId);//通知主窗口创建会话
}
