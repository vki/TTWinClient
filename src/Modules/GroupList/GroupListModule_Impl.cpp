...
void GroupListModule_Impl::getAllGroupListInfo(OUT module::GroupMap& Groups)
{
	Groups = m_mapGroups;
}
...
BOOL GroupListModule_Impl::getGroupInfoBySId(IN const std::string& sID, OUT module::GroupInfoEntity& groupInfo)
{
	...
	module::GroupMap::iterator it = m_mapGroups.find(sID);
...
void GroupListModule_Impl::onCreateDiscussionGrpDialog(void)
{
	CreateDiscussionGrpDialog* pFrame = new CreateDiscussionGrpDialog();
	if (pFrame == NULL) return;
	pFrame->Create(NULL, _T("CreateDiscussionGrpDialog"), UI_WNDSTYLE_FRAME, WS_EX_STATICEDGE | WS_EX_APPWINDOW, 0, 0, 600, 800);
	pFrame->CenterWindow();
...
void GroupListModule_Impl::_groupUnreadCntResponse(CImPdu* pdu)
{
	...
	for (UInt32 i = 0; i < pduOfflineMsgCnt->GetGroupUnreadCnt(); ++i)
	{
		...
		//m_pLogic->setDataItemTag(serv::DID_GROUPLIST_CONFIG,
		//	std::string(pInfo.group_id_url, pInfo.group_id_len),
		//	serv::TAG_GROUPLIST_OFFLINEMSG_GROUPID, TRUE);//通知Session panel有离线群消息
	}
}
...
void GroupListModule_Impl::_groupJionGroupResponse(CImPdu* pdu)
{
	...
	//if (!gID.empty())
	//{
	//	module::GroupMap::iterator itFind = m_GroupMap.find(gID);
	//	if (itFind != m_GroupMap.end())
	//	{
	//		user_id_t* puserList = pduJoinGroupRes->GetUserList();
	//		uint32_t nCount = pduJoinGroupRes->GetUserCnt();
	//		module::GroupInfoEntity& groupInfo = itFind->second;
	//		if (puserList && nCount)
	//		{
	//			for (UInt32 i = 0; i < nCount; ++i)
	//			{
	//				std::string Sid;
	//				Sid.assign(puserList[i].id_url, puserList[i].id_len);
	//				groupInfo.groupMemeberList.push_back(Sid);
	//				_MergeChangedMembers(gID, Sid, m_mapAddedMember);
	//			}
	//		}
	//		if (!groupInfo.groupMemeberList.empty())
	//		{
	//			//m_pLogic->setDataItemTag(serv::DID_GROUPLIST_CONFIG,
	//			//	gID,
	//			//	serv::TAG_GROUPLIST_MERGEADDED, TRUE);//有新的成员加入
	//		}
	//	}
	//}
}
...
void GroupListModule_Impl::_groupQuitGroupResponse(CImPdu* pdu)
{
	...
	//	if (itFind != m_GroupMap.end())
	//	{
	//		user_id_t* puserList = pduQuitGroupRes->GetUserList();
	//		uint32_t nCount = pduQuitGroupRes->GetUserCnt();
	//		module::GroupInfoEntity& groupInfo = itFind->second;
	//		size_t OrgCount = groupInfo.groupMemeberList.size();
	//		if (puserList && nCount)
	//		{
	...
	//				std::string Sid;
	//				Sid.assign(puserList[i].id_url, puserList[i].id_len);
	//				for (list<string>::iterator itDel = groupInfo.groupMemeberList.begin();
	//					itDel != groupInfo.groupMemeberList.end(); ++itDel)//删掉该成员
	//				{
	//					if (*itDel == Sid)
	//					{
	//						groupInfo.groupMemeberList.erase(itDel);
	//						_MergeChangedMembers(gID, Sid, m_mapRemovedMember);
	//						break;
	//					}
	//				}
	...
	//			}
	//		}
	//		if (OrgCount != groupInfo.groupMemeberList.size())
	//		{
	//			//m_pLogic->setDataItemTag(serv::DID_GROUPLIST_CONFIG,
	//			//	gID,
	//			//	serv::TAG_GROUPLIST_REMOVESOMEBODY, TRUE);//有成员被离开
	//		}
	//	}
	//}
}
...
void GroupListModule_Impl::_groupDiscussListResponse(CImPdu* pdu)
{
	...
	for (UInt32 i = 0; i < pList->GetGroupCnt(); ++i)
	{
		...
		{
			util::TTAutoLock lock(&m_lock);
			module::GroupMap::iterator itMap = m_mapGroups.find(groupinfo.gId);
...
void GroupListModule_Impl::onCallbackOperation(std::shared_ptr<void> param)
{
	...
	{
		module::GroupMap::iterator groupIter = m_mapGroups.find(pDownParam->m_sId);
