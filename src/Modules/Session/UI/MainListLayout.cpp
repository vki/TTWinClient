...
void MainListLayout::DoInit()
{
	...
	m_UIRecentConnectedList = static_cast<CUISessionList*>(m_pManager->FindSubControlByName(this, _T("recentlyList")));
...
void MainListLayout::_LoadAllDepartment()
{
	...
	for (module::DepartmentEntity depart : vecDeparments)
	{
		...
		for (std::string uId : depart.members)
		{
			...
			{
				SessionEntityManager::getInstance()->createSessionEntity(uId);//创建会话管理，显示的为系统时间
...
void MainListLayout::_NewMsgUpdate(std::string sId)
{
	...
	if (!psessionInfo)
	{
		APP_LOG(LOG_ERROR, _T("MainListLayout::_NewMsgUpdate，会话没创建"));
		return;
	}
	...
	else
	{
		...
		if (msg.msgStatusType == MESSAGE_TYPE_RUNTIME)
		{
			...
			if (pTTConfig && pTTConfig->sysBaseFlag & module::BASE_FLAG_NOTIPWHENNEWMSG)
			{
				...
				floatInfo.csMsgContent = util::stringToCString(msg.content);
				ReceiveMsgManage::getInstance()->parseContent(floatInfo.csMsgContent, TRUE, 400, TRUE);
...
void MainListLayout::_CreatNewDiscussGroupRes(std::string sId)
{
	...
	if (module::getGroupListModule()->getGroupInfoBySId(sId, GroupInfo))
	{
		...
		Node parent;
		if (m_GroupList->AddNodeUnderTheSid(MY_DISCUSSGROUP_ITEMID, parent))
		{
