...
CUISessionList::CUISessionList(CPaintManagerUI& paint_manager)
: UIIMList(paint_manager)
{

}

Node* CUISessionList::AddNode(const SessionListItemInfo& item, Node* parent /*= NULL*/, int index /*= 0*/)
{
	if (!parent)
		parent = root_node_;
	...
	//在UpdateItemBySId的时候会更新，所以这里可以不做
	//if (!item.folder && !item.description.IsEmpty())
	//{
	//	CLabelUI* description = static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, klastmsgControlName));
	//	if (description != NULL)
	//	{
...
BOOL CUISessionList::AddNode(const std::string& sId)
{
...
void CUISessionList::sort()
{
	SortItems(IMListItemCompareFunc, 0);
}

BOOL CUISessionList::UpdateItemBySId(const std::string& sId)
{
	if (sId.empty())
	{
		return FALSE;
...
void CUISessionList::ClearItemMsgCount(IN const std::string& sId)
{
	int nPos = 0;
	BOOL bFind = FALSE;
	CControlUI* pListElement = nullptr;
