	...
	,CString& uName,CString& pass)
{
	...
	if (_waitConnectedNotify())
	{
		...
		string password = util::cStringToString(pass);
		CImPduLoginRequest pduLogin(name.c_str(), password.c_str(), USER_STATUS_ONLINE, CLIENT_TYPE_WINDOWS);
...
void TcpClientModule_Impl::_handlePacketOperation(const char* data, UInt32 size)
{
		...
		[=]()
	{
		...
		{
			APP_LOG(LOG_ERROR, TRUE, _T("DispatchPacketTask unknown exception"));
		}
		PTR_VOID(pPdu);
